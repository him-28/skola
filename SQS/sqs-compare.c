...
int main(int argc, char *argv[]){
	...
	double distances_brute[n_sqs_atoms][n_sqs_atoms * 27];
	double distances[n_sqs_atoms][n_sqs_atoms];
...
//	printf("\nDistance table\n");
	for(int i = 0; i < n_sqs_atoms; i++){
		for(int j = 0; j < n_sqs_atoms * 27; j++){
			...
			distances_brute[i][j] = sqrt(x * x + y * y + z * z);
	...
	for(int i = 0; i < n_sqs_atoms; i++){
		for(int j = i + 1; j < n_sqs_atoms; j++){

			/* find nearest distance for atom pair (it may be in the next super cell)*/
			double mindv1, mindv2, mindv3;
			if(fabs(s_cell[i].v1 - s_cell[j].v1) > 0.5){
				mindv1 = 1.0 - fabs(s_cell[i].v1 - s_cell[j].v1);
			}
			else{
				mindv1 = fabs(s_cell[i].v1 - s_cell[j].v1);
			}

			if(fabs(s_cell[i].v2 - s_cell[j].v2) > 0.5){
				mindv2 = 1.0 - fabs(s_cell[i].v2 - s_cell[j].v2);
			}
			else{
				mindv2 = fabs(s_cell[i].v2 - s_cell[j].v2);
			}

			if(fabs(s_cell[i].v3 - s_cell[j].v3) > 0.5){
				mindv3 = 1.0 - fabs(s_cell[i].v3 - s_cell[j].v3);
			}
			else{
				mindv3 = fabs(s_cell[i].v3 - s_cell[j].v3);
			}
	...
	do {
		double match_brute[dist_list_length];
		double match[dist_list_length];
		for(int d = 1; d < dist_list_length; d++){
			int M = 0, NAB = 0;
			/* go over the table and find all pairs */
			for(int i = 0; i < n_sqs_atoms; i++){
				for(int j = 0; j < n_sqs_atoms * 27; j++){
					if(fabs(distances_brute[i][j] - dist_list[d]) < DIST_MARGIN){
						M++;
			...
			M *= 2.0;
			match_brute[d] = 1.0 - (NAB / (conc * (1.0 - conc) * M));
			//printf("%f ", match_brute[d]);
			...
			//printf("%f ", match[d]);
			if(fabs(match[d] - match_brute[d]) > MATCH_MARGIN)
				printf("nesouhlas pro vzdálenost %.3f, brute: %.6f, smart: %.6f\n", dist_list[d], match_brute[d], match[d]);
		}
