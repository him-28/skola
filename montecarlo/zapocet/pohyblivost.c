...
#define POCET_SRAZEK 10000000
#define POCET_ELEKTRONU 20
#define TOWNSENDS 1.0
...
int main () 
{
	...
	double n,t[POCET_ELEKTRONU],v[POCET_ELEKTRONU],C,emax,vmax, temp[POCET_ELEKTRONU], energyeV[POCET_ELEKTRONU], sumt[POCET_ELEKTRONU], ezstart[POCET_ELEKTRONU];
	n = P / ( 300.0 * K_B );
	...
	vec6 e[POCET_ELEKTRONU];
	uvec6 ue[POCET_ELEKTRONU];
	...
	for (int k = 0; k < POCET_ELEKTRONU; k++){
		...
		for (int i = 0; i < POCET_SRAZEK; i++) {
			...
			nova_pozice ( ue[k], uF, t[k] );
		
			v[k] = absv3 ( e[k].vx, e[k].vy, e[k].vz );
			energyeV[k] = ( 0.5 * M_E * v[k] * v[k] ) / Q_E;
			//assert ( v < vmax ); //musíme zvětšit C

			//printf ("Před srážkou: vx:%e vy:%e vz:%e\n", e.vx, e.vy, e.vz);
			temp[k] = gsl_rng_uniform (r) * ( C / v[k] );				
			if ( temp[k] < 6.0e-20 ) {
				//printf ("Pružná srážka\n");
				srazka ( ue[k], 0.0, r );
			}
			else if ( energyeV[k] > 0.20 && temp[k] < 6.0e-20 + 10 * (energyeV[k] - 0.2)* 1.0e-20 ) {
				//printf ("Nepružná srážka\n");
				srazka ( ue[k], 0.20 * Q_E , r );
			}
			else {
	...
	#pragma omp ordered
	printf ("Elektron číslo:%i, střední rychlost: %e\n", k+1, ( e[k].z - ezstart[k] ) / sumt[k] );
	
	vstd[k] = ( e[k].z - ezstart[k] ) / sumt[k];
	
	#pragma omp atomic
	sumv += ( e[k].z - ezstart[k] ) / sumt[k];
	...
	printf ("Průměrná střední rychlost %e, stdev: %e\n", avgv, prumerv );
