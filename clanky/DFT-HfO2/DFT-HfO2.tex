...
% QUESTION: Why then TB-mBJ uses LDA and GGA for the exchange part????
...
% FIXME: Add here/in Table 1 actual values???? How much is ``slightly less/more''?
...
The optical band gap of amorphous cells was estimated by an equivalent approach, i.e., fitting linear part of $\sqrt{J(E)}$ where $J(E)$ is the joint density of states. %% FIXME Should _joint_ dentsity of states be a variable of two energies J(E_1, E_2)???? Maybe some more detailed description will be benefitial here.
...
\begin{figure}
\begin{center}
% FIXME: The y-axis shows JDOS, not \sqrt{JDOS} -- please clarify, also w.r.t. the text!
...
% FIXME: 
% Pavel: now it looks like maybe we should show only the slow cooling struct. 
% David: I think that the comparison is quite interesting, and I prefer to keep it.
