...
HfO$_2$ can exist in several polymorphs, depending on growth conditions. There are three low presure polymorphs of HfO$_2$, monoclinic (spacegroup), tetragonal (spacegroup) and cubic (spacegroup), shown in figure \ref{structs}.
...
There is no associated exchange %functional ??? %%FIXME
...
%% FIXME provide a few more technical details regarding k-points, RMT, k_max, l_max, and all what is important for setting up the calculation and controlling its accuracy. Also, mention specifically which mBJ potential you use (which parametrisation).
...
%% FIXME add literature data for comparison
%% FIXME data for energies are total energie, not Ef. David needs to calculate E0 for Hf and O for both GGA and LDA and then use the above equation to get Ef
...
%%am & GGA, present & 9.922 & 92 & -10.0043
%% FIXME add/replace with Wien2k optimisations?
